...
class CopterController():
    ...
    def move_to_point(self, point):
        ...
        # TODO: реализовать полёт по точкам маршрута с соблюдением оптимальной высоты
        # vvvvv Это разкомментить
        # velocity = np.array([0.0, 0.0, 0.0])
        # velocity += self.get_horizontal_velocity(error)
        # velocity += self.get_vertical_velocity(error)
        # ^^^^^ Это разкомментить
        # vvvvv Это закомментить
        velocity = -self.p_gain * error
        velocity_norm = np.linalg.norm(velocity)
        if velocity_norm > self.max_velocity:
            velocity = velocity / velocity_norm * self.max_velocity
        # ^^^^^ Это закомментить
        self.set_vel(velocity, 0)
    ...
    def set_vel(self, velocity, yaw = None):
        if yaw is not None:
            self.pt.type_mask = self.pt.IGNORE_PX | self.pt.IGNORE_PY | self.pt.IGNORE_PZ | self.pt.IGNORE_AFX | self.pt.IGNORE_AFY | self.pt.IGNORE_AFZ | self.pt.IGNORE_YAW_RATE
            self.pt.yaw = yaw
        else:
    ...
    # TODO: сделать рассчёт вектора горизонтальной скорости
    def get_horizontal_velocity(self, error):
        return np.array([0, 0, 0])

    # TODO: рассчёт оптимальной высоты и вернуть вектор поддержаниния оптимальной высоты
    def get_vertical_velocity(self, error):
        return np.array([0, 0, 0])
...
# Получение точек маршрута TODO: реализовать построение точек для облёта заданной площади, а также найти наиболее
# TODO оптимальный маршрут полёта через них, и отсортировать их в нужном порядке. Первой и последней точкой обязательно
# TODO должна быть точка над стартовой площадкой(по сути, замкнутая задача Коммивояжера)
def get_waypoints():
    with open("../tasks/afs/gps_photo_1.zn") as f:
        s = f.read()
    ...
    for i in range(len(lst)):
        ...
        lst[i].append(400)  # += geoid_height(lst[i][0], lst[i][1])
